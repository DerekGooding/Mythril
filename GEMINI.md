...
- **Last Reviewed:** March 1, 2026 (Expander & Cadence Fixes).
...
- **Architecture**: Separated core logic into `ResourceManager`, `JunctionManager`, and `InventoryManager`. UI uses explicit JS scoping for Interop. `ResourceManager` implements locking for thread-safe state mutation.
