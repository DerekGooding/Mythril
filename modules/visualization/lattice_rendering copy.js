function renderQuestFlow() {
    ...
    const FLOW_TIER_WIDTH = 700; // Increased for more breathing room
    ...
    flowNodes.forEach(qNode => {
        ...
        if (currentView === 'advanced' && qNode.type === 'Quest' && qNode.data.quest_type === 'Recurring' && qNode.out_edges && qNode.out_edges.rewards) {
            ...
            qNode.out_edges.rewards.forEach((rew, idx) => {
                ...
                if (itemData) {
                    ...
                    // Using a slightly smaller multiplier to keep them tight to the parent
                    const verticalOffset = (idx - (rewardCount - 1) / 2) * 40;
                    
                    flowNodes.push({ 
                        ...itemData, 
                        id: terminalId, 
                        name: `${itemData.name} (${rate.toFixed(1)}/m)`, 
                        fx: qNode.fx + 300, // Positioned in the middle of the tier gap
    ...
    // 6. Final Render with Lane Assignment
    const flowNodeIdMap = new Map(flowNodes.map(n => [n.id, n]));
    
    // Group edges by their X-span to assign vertical lanes
    ...
    flowEdges.forEach(edge => {
        ...
        if (s && t) {
            ...
            spanGroups.get(key).push(edge);
        }
    });

    spanGroups.forEach((edgesInSpan, key) => {
        // Sort edges in span by their vertical midpoint to keep lanes somewhat orderly
        edgesInSpan.sort((a, b) => {
             const sa = flowNodeIdMap.get(a.source);
             const ta = flowNodeIdMap.get(a.target);
             const sb = flowNodeIdMap.get(b.source);
             const tb = flowNodeIdMap.get(b.target);
             return (sa.fy + ta.fy) - (sb.fy + tb.fy);
        });

        const [startFx, endFx] = key.split('-').map(Number);
        const laneCount = edgesInSpan.length;
        const spanWidth = endFx - startFx;
        
        // Distribution logic: Vertical segments are placed in a 'corridor' at the center of the span
        const corridorWidth = Math.min(spanWidth * 0.6, laneCount * 20);
        const laneStep = laneCount > 1 ? corridorWidth / (laneCount - 1) : 0;
        const startLaneX = startFx + (spanWidth - corridorWidth) / 2;

        edgesInSpan.forEach((edge, idx) => {
            const s = flowNodeIdMap.get(edge.source);
            const t = flowNodeIdMap.get(edge.target);
            ...
            // Lane X position
            const midX = laneCount > 1 ? startLaneX + (idx * laneStep) : startFx + spanWidth * 0.5;
            
            // Orthogonal path: Horizontal -> Vertical -> Horizontal
            const d = `M ${s.fx} ${s.fy} L ${midX} ${s.fy} L ${midX} ${t.fy} L ${t.fx} ${t.fy}`;
